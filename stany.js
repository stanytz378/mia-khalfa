                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (s.STARTING_MESSAGE === "true") {
                            ...
                            const connectionMsg = `
╔══════════════════════════╗
║     🤖 *${s.BOT_NAME || d.BOT_NAME || "MIA🍑KHALIFA"}* 🤖     ║
╚══════════════════════════╝
...
(async () => {
    // Step 1: Try to download session if SESSION_ID is provided
    await performSessionDownload();
    // Step 2: Load internal session helpers (if any)
    await loadSession();
    // Step 3: Load bot settings from database
