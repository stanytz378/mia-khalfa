...
const {
    ...
    startCleanup,
    SQLiteStore,
} = require('./stanytz/database/messageStore');
...
setInterval(() => {
    try { require("http").get(`http://localhost:${PORT}/health`); } catch (e) {}
}, 240000);
...
function setupAntiDelete(Gifted) {
    ...
    const getSender = (ms) => {
        ...
        return realJid(key.participantPn) || realJid(key.senderPn) || realJid(ms.senderPn) ||
               realJid(key.participant) || realJid(ms.participant) || key.participantPn ||
               key.participant || ms.participant ||
    ...
    const getProtocolMessage = (ms) => ms.message?.protocolMessage || ms.message?.ephemeralMessage?.message?.protocolMessage ||
        ...
        for (const ms of messages) {
            try {
                ...
                if (protocolMsg?.type === 0) {
                    ...
                    await GiftedAntiDelete(Gifted, deletedMsg, key, deleter, deletedMsg.originalSender, botJid, deleterPushName, deletedMsg.originalPushName);
...
let _newsletterCache = null, _newsletterCacheAt = 0;
...
function setupNewsletterReact(Gifted) {
    ...
    Gifted.ev.on("messages.upsert", async (mek) => {
        try {
            ...
            if (err?.code === 'ECONNRESET' || err?.code === 'ECONNREFUSED' || err?.code === 'ETIMEDOUT') _newsletterCache = null;
        }
    });
}
...
function setupPresence(Gifted) {
    Gifted.ev.on("messages.upsert", async ({ messages }) => {
        if (messages?.length > 0) await GiftedPresence(Gifted, messages[0].key.remoteJid);
    });
    Gifted.ev.on("connection.update", ({ connection }) => {
        if (connection === "open") GiftedPresence(Gifted, "status@broadcast");
    });
}
    ...
    Gifted.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (firstMsg?.message) {
            ...
            if (s.CHATBOT === "true" || s.CHATBOT === "audio") {
                GiftedChatBot(Gifted, s.CHATBOT, s.CHATBOT_MODE || "inbox", createContext, createContext2, googleTTS);
            }
        }
...
function setupAntiEdit(Gifted) {
    Gifted.ev.on("messages.update", async (updates) => {
        for (const update of updates) {
            ...
            } catch (err) { console.error("Anti-edit error:", err.message); }
        }
    });
}
...
function setupStatusHandlers(Gifted) {
    Gifted.ev.on("messages.upsert", async (mek) => {
        try {
            ...
            const s = await getAllSettings();
            const rawParticipant = msg.participant || msg.key.participantPn || msg.key.participant;
            const participantJid = await resolveRealJid(Gifted, rawParticipant);
            const shouldView = s.AUTO_READ_STATUS === "true";
            const readKey = (participantJid && participantJid !== msg.key.participant) ? { ...msg.key, participant: participantJid } : msg.key;
            ...
            if (shouldView && s.AUTO_LIKE_STATUS === "true" && participantJid) {
                ...
                const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
                const reactKey = { ...msg.key, participant: participantJid };
                await Gifted.sendMessage("status@broadcast", { react: { text: randomEmoji, key: reactKey } }, { statusJidList: [participantJid] });
            }
            if (shouldView && s.AUTO_REPLY_STATUS === "true" && !msg.key.fromMe && participantJid) {
                await Gifted.sendMessage(participantJid, { text: s.STATUS_REPLY_TEXT || DEFAULT_SETTINGS.STATUS_REPLY_TEXT }, { quoted: msg });
            }
        } catch (error) {
            const transient = error?.message?.includes("ECONNRESET") || error?.code === 428;
            if (!transient) console.error("Status handler error:", error);
        }
    });
}
    ...
    Gifted.ev.on("messages.upsert", async ({ messages, type }) => {
        ...
        if (settings.AUTO_BLOCK && sender && !isSuperUser && !isGroup) {
            ...
            if (countryCodes.some(code => sender.startsWith(code))) {
                try { await Gifted.updateBlockStatus(sender, "block"); } catch (e) {}
            }
        }
        ...
        if (bodyCmd && bodyCmd.function) {
            ...
            try {
                ...
                const conText = buildContext(ms, settings, helpers, {
                    ...
                    isBotAdmin, isAdmin, isSuperAdmin, sender, superUser, isSuperUser,
                    messageAuthor, user, pushName, args, quoted, repliedMessage, mentionedJid, tagged,
                    quotedMsg, quotedKey, quotedUser, Gifted, botId, body, command
                });
                await bodyCmd.function(from, Gifted, conText);
            } catch (error) { console.error("Body command error:", error); }
        }
            ...
            try {
                ...
                try {
                    await Gifted.sendMessage(from, {
                        ...
                        ...(await createContext(messageAuthor, { title: "Error", body: "Command execution failed" })),
                    }, { quoted: ms });
                } catch (sendErr) {}
            }
        }
...
function setupGiftedHelpers(Gifted, from) {
    ...
    Gifted.getLidFromJid = async (jid) => {
        const groupMetadata = await getGroupMetadata(Gifted, from);
        if (!groupMetadata) return null;
        const match = groupMetadata.participants.find(p => p.jid === jid || p.pn === jid || p.phoneNumber === jid || p.id === jid);
    ...
    Gifted.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        try {
            ...
            const extension = fileTypeResult?.ext || mime.split("/")[1] || (messageType === "image" ? "jpg" : messageType === "video" ? "mp4" : messageType === "audio" ? "mp3" : "bin");
...
function buildContext(ms, settings, helpers, data) {
    return {
        ...
        args: data.args, arg: data.args, quoted: data.quoted, isCmd: data.isCommand !== undefined ? data.isCommand : true,
        command: data.command || "", isAdmin: data.isAdmin, isBotAdmin: data.isBotAdmin, sender: data.sender,
        pushName: data.pushName, setSudo, delSudo, q: data.args.join(" "), reply: helpers.reply, config,
        superUser: data.superUser, tagged: data.tagged, mentionedJid: data.mentionedJid, isGroup: data.isGroup,
        groupInfo: data.groupInfo, groupName: data.groupName, getSudoNumbers, authorMessage: data.messageAuthor,
        user: data.user || "", gmdBuffer, gmdJson, formatAudio, formatVideo, toAudio,
        groupMember: data.isGroup ? data.messageAuthor : "", from: data.from, groupAdmins: data.groupAdmins,
        participants: data.participants, repliedMessage: data.repliedMessage, quotedMsg: data.quotedMsg,
        quotedKey: data.quotedKey, quotedUser: data.quotedUser, isSuperUser: data.isSuperUser,
        botMode: settings.MODE, botPic: settings.BOT_PIC, botFooter: settings.FOOTER, botCaption: settings.CAPTION,
        botVersion: settings.VERSION, ownerNumber: settings.OWNER_NUMBER, ownerName: "STANY TZ",
        botName: settings.BOT_NAME, giftedRepo: settings.BOT_REPO, packName: settings.PACK_NAME,
        packAuthor: settings.PACK_AUTHOR, isSuperAdmin: data.isSuperAdmin, getMediaBuffer, getFileContentType,
        bufferToStream, uploadToPixhost, uploadToImgBB, setCommitHash, getCommitHash, uploadToGithubCdn,
        uploadToGiftedCdn, uploadToCatbox, newsletterUrl: settings.NEWSLETTER_URL,
        newsletterJid: settings.NEWSLETTER_JID, GiftedTechApi, GiftedApiKey, botPrefix: settings.PREFIX,
